...
function plugin(opts) {
  ...
  opts = opts || {};
  opts.path = opts.path || 'tags/:slug/index.html';
  opts.pathPage = opts.pathPage || 'tags/:slug/:num/index.html';
  ...
  return function(files, metalsmith, done) {
    ...
    function getFilePath(path, opts) {
      return path.replace(/:num/g, opts.num).replace(/:tag/g, opts.tag).replace(/:slug/g, opts.slug);
    }
    ...
    Object.keys(files).forEach(function(file) {
      ...
      if (tagsData) {
        ...
        data[opts.handle] = tagsData.map(function(tag) {
            ...
            meta.path = getFilePath(opts.path, meta).replace('index.html', '');
        ...
        data[opts.handle].forEach(function(tag) {
          ...
          tagList[tag.slug].files.push(data);
        });
      }
    });
    ...
    Object.keys(tagList).forEach(function(slug) {
      var tag = tagList[slug],
          // Sort tags via opts.sortBy property value.
          posts = tag.files.sort(sortBy);
      ...
      metadata[opts.metadataKey][tag.slug] = posts;
